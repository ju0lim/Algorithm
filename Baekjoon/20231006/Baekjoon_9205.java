...
public class Main
{
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        while (testCount-- > 0)
        {
            ...
            Point[] locations = new Point[storeCount + 2];
            ...
            {
                st = new StringTokenizer(reader.readLine());
                locations[i+1] = new Point(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()));
            }

            st = new StringTokenizer(reader.readLine());
            locations[storeCount+1] = new Point(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()));
            ...
            for (int i = 0; i < storeCount + 2; i++)
            {
                for (int j = i + 1; j < storeCount + 2; j++)
                {
                    if (getDistance(locations[i], locations[j]) <= 1000)
                    {
                        graph.get(i).add(j);
                        graph.get(j).add(i);
                    }
            ...
            String result = canArrive(graph, storeCount + 1) ? "happy" : "sad";
    ...
    public static boolean canArrive(ArrayList<ArrayList<Integer>> graph, int targetIndex)
    {
        Queue<Integer> queue = new LinkedList<>();
        queue.offer(0);
        boolean[] visit = new boolean[targetIndex+1];
